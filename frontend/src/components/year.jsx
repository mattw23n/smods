...
const Term = ({ term, plan, mods, setMods, type, setValidationResponse, isEditMode }) => {
    ...
    const handleDragEnd = async (e) => {
        ...
        if (modToTransfer && originTerm !== term.toString()) {
            ...
            if (originTerm !== 0) {
                ...
                try {
                    const deleteResponse = await fetch(`http://localhost:8080/api/users/${plan.userId}/plans/${plan.planId}/update-module?moduleId=${moduleId}&term=${originTerm}&isAdding=false`, {
                        method: 'PUT',
                        headers: {
        ...
        try {
            const addResponse = await fetch(`http://localhost:8080/api/users/${plan.userId}/plans/${plan.planId}/update-module?moduleId=${moduleId}&term=${term}&isAdding=true`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('jwt')}`
    ...
    const handleDelete = async (moduleId) => {
        try {
            const response = await fetch(`http://localhost:8080/api/users/${plan.userId}/plans/${plan.planId}/update-module?moduleId=${moduleId}&term=&isAdding=false`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('jwt')}`
